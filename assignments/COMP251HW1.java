...
public class COMP251HW1 {
	...
	public double divisionMethodImpl (int r, int n, int w) {
		...
		boolean[] tracker = new boolean[(int) D];
		
		for (int i = 0; i < n; i++) {
			int key = (int) (randomNumbers[i] % D);
			if (tracker[key]) {
				collisions++;
			} else {
				tracker[key] = true;
	...
	public double[] multiplicationMethod (int r, int w) {
		double[] ys = new double[ns.length];
		double y;
		double A = generateRandomNumberiInRange(Math.pow(2, w -1), Math.pow(2, w)); // This line should be updated to assign a valid value to A
		for (int it = 0, len = ns.length; it < len; it++) {
			int m = (int)(Math.pow(2,r));
			int n = (int)(ns[it]*m);
	...
	public double multiplicationMethodImpl (int r, int n, int w, double A) {
		...
		for (int i = 0; i < n; i++) {
			int key = (int) ((A * randomNumbers[i]) % Math.pow(2, w)) >> (w - r);
			if (tracker[key]) {
				collisions++;
			} else {
				tracker[key] = true;
